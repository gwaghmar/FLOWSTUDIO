...
import { resolveProviderIconPath } from "@/lib/diagrams/provider-icons";
...
function IconNode({ data }: { data: IconNodeData }) {
  ...
  const providerIconPath = resolveProviderIconPath(data.provider, data.service);
  const brand = providerIconPath ? null : resolveBrandIcon(data.service);
  const glyph = providerIconPath
    ? // eslint-disable-next-line @next/next/no-img-element
      <img src={providerIconPath} alt="" width={22} height={22} />
    : brand
    ? (
  ...
  return (
    <div className="flex min-w-[120px] flex-col items-center gap-1.5 rounded-xl border-2 border-slate-200 bg-white px-4 py-3 shadow-md transition-all hover:shadow-lg">
      ...
      <span style={providerIconPath || brand ? undefined : { color }} className="flex h-7 w-7 items-center justify-center">{glyph}</span>
